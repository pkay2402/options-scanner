...
class SummarizeCommands(commands.Cog):
    ...
    async def summarize(self, interaction: discord.Interaction, symbol: str, days: Optional[int] = 1):
        ...
        try:
            # Validate days
            if days < 1 or days > 5:
                await interaction.followup.send("❌ Days must be between 1 and 5")
                return
            ...
            if summary['total_signals'] == 0:
                ...
                await interaction.followup.send(
                    f"📭 No signals found for **{symbol}** {period_text}.\n"
    ...
    async def timeline(self, interaction: discord.Interaction, symbol: str, days: Optional[int] = 5):
        ...
        try:
            ...
            if not timeline:
                await interaction.followup.send(
                    f"📭 No activity found for **{symbol}** in the last {days} days."
                )
                return
            
            # Create embed
            embed = discord.Embed(
                title=f"📅 {symbol} Activity Timeline",
            ...
            for day_data in timeline[:5]:  # Max 5 days
                ...
                embed.add_field(
                    name=f"📆 {date} ({len(signals)} signals)",
...
def format_summary_embed(symbol: str, summary: dict, days: int) -> discord.Embed:
    ...
    embed = discord.Embed(
        title=f"📊 {symbol} Trading Summary - {period_text}",
    ...
    if summary['by_type']:
        ...
        if 'WHALE' in summary['by_type']:
            ...
            whale_text = f"🐋 **Whale Flow**: {whale_data['count']} signals"
        ...
        if 'ZSCORE' in summary['by_type']:
            ...
            zscore_text = f"📊 **Z-Score**: {zscore_data['count']} signals"
        ...
        if 'TOS' in summary['by_type']:
            ...
            tos_text = f"🎯 **TOS Alerts**: {tos_data['count']} signals"
        ...
        if 'ETF_MOMENTUM' in summary['by_type']:
            ...
            etf_text = f"📈 **ETF Momentum**: {etf_data['count']} signals"
    ...
    embed.set_footer(text=f"Total Signals: {summary['total_signals']} | {bullish}🟢 {bearish}🔴 {neutral}⚪")
...
def generate_insights(symbol: str, summary: dict, days: int) -> str:
    ...
    if 'WHALE' in summary['by_type']:
        ...
        if whale_count >= 3:
            insights.append(f"🐋 **Heavy whale activity** ({whale_count} large trades detected)")
        
        subtypes = summary['by_type']['WHALE'].get('subtypes', {})
        if 'BUY' in subtypes and subtypes['BUY'] > subtypes.get('SELL', 0):
            insights.append("💪 Whales are accumulating - strong buying pressure")
        elif 'SELL' in subtypes and subtypes['SELL'] > subtypes.get('BUY', 0):
            insights.append("⚠️ Whales are distributing - selling pressure detected")
    ...
    if 'ZSCORE' in summary['by_type']:
        ...
        if 'OVERBOUGHT' in subtypes:
            insights.append(f"📊 Entered overbought zone - potential pullback ahead")
        elif 'OVERSOLD' in subtypes:
            insights.append(f"📊 Hit oversold levels - bounce opportunity")
        
        if 'RECOVERY' in subtypes:
            insights.append(f"💫 Recovery momentum detected - bouncing from lows")
        
        # Check for reversals
        if 'SELL_SIGNAL' in subtypes and 'OVERBOUGHT' in subtypes:
            insights.append(f"🔴 Reversal signal: Crossed down from overbought")
        elif 'BUY_SIGNAL' in subtypes and 'OVERSOLD' in subtypes:
            insights.append(f"🟢 Reversal signal: Crossed up from oversold")
    ...
    if 'TOS' in summary['by_type']:
        ...
        if tos_count >= 2:
            if 'BULLISH' in subtypes or 'BUY' in subtypes:
                insights.append(f"🎯 Multiple bullish TOS setups forming")
            elif 'BEARISH' in subtypes or 'SELL' in subtypes:
                insights.append(f"🎯 Multiple bearish TOS setups forming")
    ...
    if 'ETF_MOMENTUM' in summary['by_type']:
        ...
        if 'STRONG_MOMENTUM' in subtypes:
            insights.append(f"🚀 Strong momentum trend in play")
        elif 'MOMENTUM_SHIFT' in subtypes:
            insights.append(f"🔄 Momentum shift detected - trend change possible")
    
    # Overall pattern analysis
    if summary['total_signals'] >= 5:
        if summary['by_direction']['BULLISH'] >= summary['total_signals'] * 0.7:
            insights.append(f"🎯 **Consistent bullish signals** - strong uptrend")
        elif summary['by_direction']['BEARISH'] >= summary['total_signals'] * 0.7:
            insights.append(f"🎯 **Consistent bearish signals** - strong downtrend")
        elif summary['by_direction']['NEUTRAL'] >= summary['total_signals'] * 0.6:
            insights.append(f"⚖️ Sideways action - waiting for directional move")
    
    # Activity level
    if days > 1:
        avg_signals_per_day = summary['total_signals'] / days
        if avg_signals_per_day >= 3:
            insights.append(f"⚡ High activity stock - {avg_signals_per_day:.1f} signals/day")
        elif summary['total_signals'] <= 2:
            insights.append(f"📉 Low activity - limited signal generation")
    
    if not insights:
        insights.append("📊 Continue monitoring for new signals")
