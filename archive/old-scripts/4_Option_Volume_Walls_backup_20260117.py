...
if st.session_state.run_analysis:
    ...
    # Create a persistent log area that won't disappear
    debug_log = st.empty()
    debug_messages = []
    
    def log_step(msg):
        debug_messages.append(f"✓ {msg}")
        debug_log.code("\\n".join(debug_messages), language=None)
    
    log_step("Step 1: Starting analysis...")
    ...
    with st.spinner(f"🔄 Analyzing option volumes for {symbol}..."):
        try:
            # ===== VISIBLE DEBUG =====
            log_step("Step 2: Inside try block, preparing to fetch data...")
            ...
            log_step(f"Step 3: Calling get_market_snapshot({symbol}, {exp_date_str})...")
            logger.info(f"[DEBUG] Starting analysis for {symbol}, expiry: {exp_date_str}")
            
            snapshot = get_market_snapshot(symbol, exp_date_str)
            
            log_step(f"Step 4: get_market_snapshot returned. snapshot={'exists' if snapshot else 'None'}")
            ...
            log_step(f"Step 5: Snapshot received! Price: ${snapshot.get('underlying_price', 'N/A')}")
            ...
            log_step(f"Step 6: Data extracted. Options chain exists: {options is not None}")
            ...
            if options:
                ...
                log_step(f"Step 7: Found {len(call_exp_dates)} call expiries, {len(put_exp_dates)} put expiries")
            ...
            log_step(f"Step 8: Calculating Most Valuable Strike...")
            
            # Calculate Most Valuable Strike
            mvs = calculate_most_valuable_strike(options, underlying_price)
            
            log_step(f"Step 9: MVS calculated. Rendering price info...")
            ...
            log_step(f"Step 10: Calling calculate_option_walls...")
            ...
            log_step(f"Step 11: calculate_option_walls returned. levels={'exists' if levels else 'None'}")
            ...
            log_step(f"✅ Step 12: All data ready! Call Wall: ${levels['call_wall']['strike']}, Put Wall: ${levels['put_wall']['strike']}")
            ...
            try:
                log_step(f"❌ CRASHED at last step. Error: {str(e)}")
            except:
                pass
