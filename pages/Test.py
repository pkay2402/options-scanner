...
def create_macd_chart(price_history, symbol):
    ...
    try:
        ...
        # --- Detect MACD/Signal crossovers and add arrow markers ---
        try:
            df['macd_prev'] = df['macd'].shift(1)
            df['signal_prev'] = df['signal'].shift(1)
            ...
            if up_cross.any():
                fig.add_trace(go.Scatter(
                    ...
                    y=df.loc[up_cross, 'macd'],
                    mode='markers',
                    name='Bull Cross',
                    marker=dict(symbol='triangle-up', color='#22c55e', size=12),
                    hovertemplate='<b>Bull Cross</b><br>%{x|%I:%M %p}<br>MACD: %{y:.4f}<extra></extra>'
                ))

            if down_cross.any():
                fig.add_trace(go.Scatter(
                    x=df.loc[down_cross, 'datetime'],
                    y=df.loc[down_cross, 'macd'],
                    mode='markers',
                    name='Bear Cross',
                    marker=dict(symbol='triangle-down', color='#ef4444', size=12),
                    hovertemplate='<b>Bear Cross</b><br>%{x|%I:%M %p}<br>MACD: %{y:.4f}<extra></extra>'
                ))
        except Exception:
            # If anything goes wrong in crossover plotting, continue without markers
            logger.exception('Error plotting MACD cross markers')
