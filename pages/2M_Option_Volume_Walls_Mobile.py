"""
Option Volume Walls & Levels - MOBILE VERSION
Mobile-optimized interface for option volume analysis
"""
...
st.set_page_config(
    page_title="Volume Walls Mobile",
    page_icon="📱",
...
<style>
    ...
    /* Hide some less critical elements on small screens */
...
st.title("📱 Volume Walls Mobile")
st.caption("Mobile-optimized option volume analysis")
...
with st.expander("⚙️ Settings", expanded=True):
    ...
    multi_expiry = st.checkbox("📅 Compare Multiple Expirations", value=False)
    show_heatmap = st.checkbox("🔥 Show Gamma Heatmap", value=False)
...
def create_intraday_chart_with_levels(price_history, levels, underlying_price, symbol):
    ...
    try:
        ...
        fig.update_layout(
            ...
            xaxis_title="Time (ET)",
            yaxis_title="Price ($)",
            height=400,  # Mobile: reduced from 550
...
def create_macd_chart(price_history, symbol):
    ...
    try:
        ...
        fig.update_layout(
            ...
            xaxis_title="Time (ET)",
            yaxis_title="MACD",
            height=200,  # Mobile: reduced from 250
            template='plotly_white',
            hovermode='x unified',
            xaxis=dict(
                type='date',
...
def create_gamma_heatmap(options_data, underlying_price, num_expiries=6):
    ...
    try:
        ...
        fig.update_layout(
            ...
            height=450,  # Mobile: reduced from 650
            template='plotly_white',
            font=dict(size=13),
...
def create_volume_profile_chart(levels, underlying_price, symbol):
    ...
    try:
        ...
        fig.update_layout(
            ...
            yaxis_title="Strike Price ($)",
            height=600,  # Mobile: reduced from 900
            template='plotly_white',
...
if st.session_state.run_analysis:
    with st.spinner(f"🔄 Analyzing option volumes for {symbol}..."):
        try:
            ...
            corner_col3, corner_col4 = st.columns(2)
            ...
            if chart and show_gex_sidebar and strike_gex is not None and len(strike_gex) > 0:
                ...
                fig.update_layout(
                    height=450,  # Mobile: reduced from 600
                    template='plotly_white',
                    hovermode='closest',
                    showlegend=True,
                    xaxis_rangeslider_visible=False,
            ...
            # Mobile: Stack charts vertically in collapsible sections
            st.markdown("---")
            ...
            with st.expander("💰 Net Premium Flow", expanded=False):
                ...
                if multi_snapshot and multi_snapshot['options_chain']:
                    ...
                    if premium_map:
                        ...
                        st.info("Net premium heatmap not available")
                else:
                    st.info("Could not fetch multi-expiry options data")
